  ...
  router.get("/", async (req, res) => {
    try {
      ...
      console.error("Unexpected employee list error:", error);
  ...
  router.get("/:id", async (req, res) => {
    try {
      ...
      console.error("Unexpected employee lookup error:", error);
  ...
  router.post("/", async (req, res) => {
    try {
      ...
      const cleanDepartment = cleanOptionalString(department);
      const cleanTitle = cleanOptionalString(title);
      const cleanEmployeeCode = cleanOptionalString(employee_code);
      const cleanJoiningDate = cleanOptionalString(joining_date);
      const cleanStatus =
      ...
      const validationErrors = validateEmployeeFields({
        full_name: cleanName,
        email: cleanEmail,
      ...
      if (emailLookupError) {
        ...
        return res.status(500).json({
          message: "Could not check existing employee",
      ...
      if (cleanEmployeeCode) {
        ...
        if (codeLookupError) {
          ...
          return res.status(500).json({
            message: "Could not check employee code",
  ...
  router.post("/bulk", async (req, res) => {
    try {
      ...
      incomingEmployees.forEach((employee, index) => {
        ...
        const email = cleanEmailValue(employee?.email);

        const department = cleanOptionalString(
          employee?.department
        );
        ...
        const employeeCode = cleanOptionalString(
          employee?.employee_code
        );

        const joiningDate = cleanOptionalString(
        ...
        const lastWorkingDate = cleanOptionalString(
          employee?.last_working_date
        );

        const address = cleanOptionalString(
          employee?.address
        );
        ...
        if (
          ...
          employeeCodeSet.has(normalizedEmployeeCode)
        ) {
          errors.push(
      ...
      if (validEmployees.length > 0) {
        const emails = validEmployees.map(
          (employee) => employee.email
        );

        const employeeCodes = validEmployees
        ...
        if (existingEmailError) {
          ...
          return res.status(500).json({
            message: "Could not check existing employees",
            detail: existingEmailError.message,
          });
        }
        ...
        if (employeeCodes.length > 0) {
          ...
          if (existingCodeError) {
            ...
            return res.status(500).json({
              ...
              detail: existingCodeError.message,
            });
          }
        ...
        validEmployees.forEach((employee) => {
          ...
          if (
            employeeCodeKey &&
            existingCodes.has(employeeCodeKey)
          ) {
            errors.push(
      ...
      for (
        ...
        const chunk = validEmployees.slice(
          i,
          i + chunkSize
        );
        ...
        if (error) {
          ...
          return res.status(500).json({
            message: "Could not import employees",
            detail: error.message,
